...
from products.platform_features.backend.property_access_control import (
    PropertyAccessLevel,
...
class TestQueryTimeFeatureGate(BaseTest):
    """
    When the organization does NOT have ACCESS_CONTROL, query-time enforcement must
    short-circuit — every property resolves to the default access level and no property is
    restricted, even when rules exist in the DB. (The rules stay in the DB so behavior
    restores cleanly if the org re-subscribes.)
    """

    def setUp(self):
        super().setUp()
        # Intentionally do NOT enable ACCESS_CONTROL.
        self.event_prop = PropertyDefinition.objects.create(
            team=self.team,
    ...
    def test_get_property_access_level_also_bypasses_user_overrides(self):
        self._create_none_rule(self.event_prop)
        PropertyAccessControl.objects.create(
            team=self.team,
            property_definition=self.event_prop,
    ...
    def test_get_restricted_property_names_returns_empty_without_feature(self):
    ...
    def test_get_non_writable_property_names_returns_empty_without_feature(self):
        from products.platform_features.backend.property_access_control import get_non_writable_property_names

        PropertyAccessControl.objects.create(
            team=self.team,
            property_definition=self.person_prop,
            access_level=PropertyAccessLevel.READ.value,  # read-only, would normally be non-writable
        )
