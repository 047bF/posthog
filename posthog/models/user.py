...
class User(AbstractUser, UUIDTClassicModel, ModelActivityMixin):  # type: ignore[django-manager-missing]
    ...
    def get_github_login(self) -> str | None:
        ...
        2. ``UserSocialAuth`` (provider=github) — fallback for users who log in
           with GitHub but haven't set up a ``UserIntegration`` yet.
        3. Team ``Integration`` ``connecting_user_github_login`` — legacy fallback
           from before the user integration model existed.
        """
